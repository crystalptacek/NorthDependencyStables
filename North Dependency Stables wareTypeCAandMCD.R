...
setwd("P:/Projects/2014/North Dependency Stables/R Code")
...
# Updated for North Dependency Stables report: CLP 2.21.2019
...
p <- ggplot(data=inertia , aes(x= 1:length(Inertia), y=Inertia)) +
  ...
  labs( title="North Dependency Stables", x="Dimension", y='Porportion of Inertia' ) +
...
p5a <- p5 + geom_vline(xintercept=c(- 1, 2 ), colour = "gray", linetype = "dashed",
                       size=1)
p5a
...
write.csv(phaseAssignments, file='ContextPhases_NDepStables.csv')

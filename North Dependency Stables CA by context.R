...
# Updated to just North Dependency Stables CLP 10.24.2018
# RUN BY CONTEXT FOR THE UNIT
...
wareTypeData1 <-
  mutate(wareTypeDataY, unit=ifelse((FeatureNumber == ''),
                              paste(Context), paste(FeatureNumber)))
...
#delete any outliers
WareByUnitT1 <-subset(WareByUnitT0, !WareByUnitT0$unit %in% c('2430C', '2435A'))

#Ok now let's get rid of all the columns (ware types) where totals < 0
#WareByUnitT2<-WareByUnitT0[, colSums(WareByUnitT0 != 0) > 0]
...
p1 <- ggplot(rowscores, aes(x=rowscores$Dim1,y=rowscores$Dim2))+
  ...
  theme_classic()+
  labs(title="North Dependency Stables", x="Dimension 1", y="Dimension 2")+
...
ggsave("NDepStables_Figure1Dim1Dim2_2018cxt.png", p1, width=10, height=7.5, dpi=300)
...
#require(ggplot2)
#library(ggrepel)
#p1a <- ggplot(rowscores, aes(x=rowscores$Dim1,y=rowscores$Dim3))+
#  geom_point(shape=21, size=5, colour="black", fill="cornflower blue")+
#  geom_text(aes(label=CA_MCD_Phase1$unit),vjust=-.6, cex=5)+
#  geom_text_repel(aes(label=rownames(rowscores)), cex=4) +
#  theme_classic()+
#  labs(title="North Dependency Stables", x="Dimension 1", y="Dimension 3")+
#  theme(plot.title=element_text(size=rel(2.25), hjust=0.5),axis.title=element_text(size=rel(1.75)),
# axis.text=element_text(size=rel(1.5)))
#p1a
#save the plot for website chronology page/presentations
...
p2
#save the plot for website chronology page/presentations
ggsave("SouthPavilion_Figure2WareTypes_2018cxt.png", p2, width=10, height=7.5, dpi=300)
...
p3 <- ggplot(rowscores, aes(x=rowscores$Dim1,y=MCDByUnit$MCDs$blueMCD))+
  ...
  theme_classic()+
  labs(title="North Dependency Stables", x="Dimension 1", y="BLUE MCD")+
  theme(plot.title=element_text(size=rel(2.25), hjust=0.5),axis.title=element_text(size=rel(1.75)),
        axis.text=element_text(size=rel(1.5)))
...
#save the plot for website chronology page/presentations
ggsave("NDepStables_Dim1BLUEMCD_2018cxt.png", p3, width=10, height=7.5, dpi=300)
...
p4 <- ggplot(rowscores, aes(x=rowscores$Dim2,y=MCDByUnit$MCDs$blueMCD))+
  ...
  # geom_text_repel(aes(label=rownames(rowscores)), cex=6) +
  theme_classic()+
  labs(title="North Dependency Stables", x="Dimension 2", y="BLUE MCD")+
  theme(plot.title=element_text(size=rel(2.25), hjust=0.5),axis.title=element_text(size=rel(1.75)),
        axis.text=element_text(size=rel(1.5)))
p4 
cor.test(ca3$rowcoord[,2],MCDByUnit$MCDs$blueMCD, method="kendall")
#ggsave("NDepStables_Dim2BLUEMCD.png", p4, width=10, height=7.5, dpi=300)
...
#weighted.hist(CA_MCD$blueMCD, CA_MCD$count, breaks=seq(1780,1840,10), col='lightblue')
...
p5 <- ggplot(CA_MCD, aes(x=CA_MCD$dim1Scores, weight=CA_MCD$count/sum(CA_MCD$count)))+
  ...
  theme_classic()+
  labs(title="North Dependency Stables", x="Dimension 1", y="Density")+
  theme(plot.title=element_text(size=rel(2.25), hjust=0.5),axis.title=element_text(size=rel(1.75)),
        axis.text=element_text(size=rel(1.5)))+
...
p5a
ggsave("NDepStables_Histogram_2018cxt.png", p5, width=10, height=7.5, dpi=300)
...
p6
#save the plot for website chronology page/presentations
ggsave("NDepStables_Dim1MCDcolor_2018.png", p6, width=10, height=7.5, dpi=300)
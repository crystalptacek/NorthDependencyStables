...
wareTypeData<-dbGetQuery(DRCcon,'
                         ...
                         WHERE "public"."tblContext"."ProjectID" = \'52\'
                         ')
...
#  
wareTypeData$unit <- wareTypeData$QuadratID
...
# wareTypeData <- filter(wareTypeData, ! wareTypeData$QuadratID   %in% 
#                          c('082%'))

# Remove specific contexts
#wareTypeData <- filter (wareTypeData, !unit  
...
# wareTypeData <-  filter(wareTypeData, ! wareTypeData$Ware  %in%  
...
WareByUnit <- wareTypeData %>% group_by(Ware,unit) %>% 
  summarise(count = sum(Quantity))
...
# Now we need to drop everything that doesn't have a unit
WareByUnitT <- filter(WareByUnitT, unit!='')
...
p1 <- ggplot(rowScores, aes(x=Dim1,y=Dim2))+
  ...
  labs(title="North Dependency Stables", 
...
#ggsave("NDepStables_Figure1Dim1Dim2_2018quad.png", p1, width=10, height=7.5, dpi=300)
...
p2
#save the plot for website chronology page/presentations
#ggsave("NDepStables_Figure2WareTypes_2018quad.png", p2, width=10, height=7.5, dpi=300)
...
# save the plot for website chronology page/presentations
# ggsave("NDepStables_Dim1BLUEMCD_2018quad.png", p3, width=10, height=7.5, dpi=300)
...
p5a <- p5 + geom_vline(xintercept=c(-5), colour = "gray", linetype = "dashed",
                       size=1)
p5a

# save the plot for website chronology page/presentations
# ggsave("NDepStables_Histogram.png", p5a, width=10, height=7.5, dpi=300)
# 
...
MCDByPhase
#Export data
#write.csv(CA_MCD_Phase, file='CA_MCD_Phase_NDepStables.csv')
...
#save the plot for website chronology page/presentations
#ggsave("NDepStablesByQuadrat.png", p6, width=10, height=7.5, dpi=300)